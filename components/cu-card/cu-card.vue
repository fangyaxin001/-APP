<template>
	<view :class="comment?'pb-100':''">
				...
				<view class="cu-item" @click.stop="comment?commentFun(true):jumpDetail(showData.aid)">
					...
					<view class="flex align-center padding-left padding-top">
						...
						<view class="flex align-center justify-end text-grey text-sm text-right flex-sub text-df">
							...
							<view @click.stop="jubao" v-if="reportFlag" style="font-size: 22rpx;margin-left: 20rpx;" class="padding-tb-xs flex align-center handle">
								<image style="width: 22rpx; height: 25rpx;" src="../../static/imgs/icon/jb.png" mode=""></image>
								举报
							</view>
						</view>
					</view>
		...
		<uni-popup ref="popup3" type="dialog">
			<uni-popup-dialog type="info" title="确定要举报该论坛吗？" :before-close="true" @confirm="confirmsty" @close="close"></uni-popup-dialog>
		</uni-popup>
		...
		<view class="cu-modal bottom-modal" :class="modalName=='headModal'?'show':''" @click.stop="modalName=''"
		 @touchmove.stop.prevent>
			<view class="cu-dialog bg-eded modalName" @click.stop="stopFun">
				...
				<view class="text-center text-lg bg-white item text-red" @click.stop="complainFun">
					不感兴趣
				</view>
				<view class="text-center text-lg bg-white item text-red" @click.stop="complainFun">
					暴力血腥
				</view>
				<view class="text-center text-lg margin-top-sm bg-white item" @click.stop="modalName='' ">
					取消
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		created() { 
		 this.$api.post(global.apiUrls.loginFlag,  )
				.then(res => {
					console.log(res)
					console.log( res.data.data.code)
					if(res.data.data.code){
						this.showData.is_me = true
					}
					
					this.reportFlag =  res.data.data.code ? true:false
				})
		},
		methods: {
				...
				this.$api.post(global.apiUrls.deleteBBS, {
					...
					if(this.reportFlag){
						if (status) {
							...
							this.$emit('refreshdata', id,this.reportFlag);
						}
					}
				})
			},
			...
			confirmsty(done){
			  console.log("The testers are SB")
