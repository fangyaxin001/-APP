	...
	<view class="search">
		...
		<view class="" v-if="(listData['group_array'] && listData['group_array'].length>0) || (listData['friend_data'] && listData['friend_data'].length>0)">
			<view class="content padding-bottom-sm bg-white" v-if="listData['friend_data'].length>0">
				...
				<view class="cu-form-group" @click="handleJump" :data-url="`/pages/chat/chat?id=${item.id}&type=1&title=${item.nickname}&head_img=${item.head_img}`" v-for="(item,index) in listData['friend_data']" :key="index">
			...
			<view class="content margin-top padding-bottom-sm bg-white"  v-if="listData['group_array'].length>0">
				...
				<view class="cu-form-group" @click="handleJump" :data-url="`/pages/chat/chat?id=${item.id}&type=2&title=${item.group_title}&group_id=${item.group_id}`" v-for="(item,index) in listData['group_array']" :key="index">
