			 ...
			 v-for="(item,index) in listData.front" :key="index">
				...
				<view class="flex-sub cu-form-group justify-between p-0">
					<view class="text-lg">
						...
						<view class="text-gray text-sm text-999 text-cut" style="width: 400rpx;">{{item.msg}}</view>
					</view>
		...
		methods: {
			...
			refuseFriend(id) {
				...
				this.$api.post(global.apiUrls.refuseFriend, {
					...
					var res = res.data;
					if (res.code == 1) {
						// 环信拒绝好友请求（如果对方在好友列表页面，那就通知他更新）
						WebIM.conn.declineInvitation('heiban' + id);
						_this.$message.info(res.msg);
						this.getApplyFriend();
					} else {
						_this.$message.info(res.msg)
