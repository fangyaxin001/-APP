	...
	<view class="group">
		...
		<view class="group-head padding-bottom">
			...
			<view class="head-items" v-if="isJoin==1">
				<view class="head-img">
					<image src="/static/ye_icon/3599@2x.png" @click.stop="handleJump" :data-url="'/pages/friendData/chooseFriends?type=1&id='+groupID+'&hxid='+groupInfo.group_id"></image>
				</view>
			</view>
			...
			<view class="head-items" v-for="(item,index) in [1,2,3,4]" :key="'key'+item">
			</view>
			...
			<view class="" v-if="isJoin==1">
				...
				<view class="apply">
					<view class="group-box padding-lr mt-0">
						...
						<view class="group-row" @click.stop="handleJump" :data-url="`/pages/friendData/findRecords?type=2&group_id=${groupID}&fromid=${groupInfo.group_id}`">
							<view class="group-row-tit">
								<text>查找聊天记录</text>
							</view>
							<view class="group-row-info">
	...
	export default {
		...
		methods: {
			...
			confirm(done) {
				...
				var sessionKey = _this.groupInfo.group_id + 'heiban' + global.userInfo.id;
			...
			switchFun(e, ind) {
				...
				this.$api.post(global.apiUrls.set_group_notice, data).then(res => {
					...
					if (res.code == 1) {
						...
						if (ind == 2) {
							var options = {
								groupId: _this.groupInfo.group_id, //群组id
							};
							if (switchVal) {
								//群组中禁言所有成员
			...
			notDisturbFun(e) {
				...
				this.$api.post(global.apiUrls.set_group_nickname, {
					...
					if (res.code == 1) {
						// 更新消息列表
						console.log('修改消息免打扰状态')
						disp.fire("message.show", '修改消息免打扰状态');
					} else {
						// 更改失败 switch 变回原来的
						setTimeout(() => {
