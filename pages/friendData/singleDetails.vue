	...
	<view class="group">
		...
		<view class="apply margin-top-sm bg-white overflow">
			<view class="group-box padding-lr mt-0">
				<view class="group-row" @click.stop="handleJump" :data-url="`/pages/friendData/findRecords?type=1&fromid=${'heiban'+id}&headimg=${userData.head_img}&nickname=${userData.friend_user_nickname}`">
	...
	export default {
		...
		methods: {
			...
			confirmTwo(done) {
				...
				var sessionKey = 'heiban' + global.userInfo.id + 'heiban' + _this.userData.id;
