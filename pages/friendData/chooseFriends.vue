						...
						<view class="cu-form-group border-0" @tap="handleJump" :data-url="'/pages/friendData/friendData?id='+friend.id+'&close=1'"
						 v-for="(friend,index) in item.data" :key="index">
							<image class="checked" :src="selectArr.indexOf(friend.id)!=-1?'/static/xs_icon/shoseSel.png':'/static/xs_icon/chose.png'"
							 mode="" @click.stop="selectFun(friend.id,friend.user_nickname)"></image>
	...
	export default {
		...
		methods: {
			...
			selectFun(id,name) {
				...
				var selectNameArr = this.selectNameArr || [];
				if (ind == -1) {
					this.selectArr.push(id);
					this.selectNameArr.push(name);
				} else {
					this.selectArr.splice(ind, 1)
					this.selectNameArr.splice(name, 1)
				}
					...
					this.$api.post(api, {
						...
						if (res.data.code == 1) {
							...
							if (_this.type == 1) {
								...
								var customExts = {
									text: _this.selectNameArr.join(',') + ' 加入群聊'
